 ...
 * Includes basic functions such as database_query, database_count,
 * add_to_list, opt_in_email, add_contact
 */
class Infusionsoft {
	 ...
	 *
	 * @param array $args API Action ( string ) / Request parameters ( array )
	 *     / Independent parameters ( boolean )
	 *
	 * @return string
	 */
	public function create_xml_request( $args ) {
		...
		$whitelisted_types = [ 'string', 'int', 'array', 'struct' ];
		...
		foreach ( $args['params'] as $name => $settings ) {
			...
			if ( ! $is_no_value_tags ) {
				$value = sprintf( $value_template, $value, sanitize_text_field( $type ) );
			}

			$parameters .= sprintf( $param_template, $value );

			if ( ! $use_independent_parameters ) {
				$value = ! $is_no_value_tags ? sprintf( $value_template, sanitize_text_field( $settings['value'] ), sanitize_text_field( $type ) ) : sanitize_text_field( $settings['value'] );

				$parameters .= sprintf( $param_template, $value );
			}
		}

		$request = sprintf( $this->_xml_template, esc_html( $args['action'] ), $parameters );

		return $request;
	}

	/**
	 * Perform the test request to infusionsoft server to check the API key and
	 * App ID
	 */
	public function connection_check() {
		...
		if ( 0 === $result || empty( $result ) ) {
			return esc_html__( 'Invalid App ID', 'xo' );
		}

		if ( isset( $result->fault ) ) {
			return esc_html__( 'Invalid Api Key', 'xo' );
		}
	...
	public function database_query( $table_name = '', $limit, $page, $search_fields, $return_fields ) {
		...
		if ( ! empty( $search_fields ) ) {
			foreach ( $search_fields as $field_name => $value ) {
				$value = sprintf( $value_xml_template, esc_html( $value ) );

				$search_fields_xml .= sprintf( '<value>
						<struct>
							<member>
								<name>%1$s</name>
								%2$s
					...
					</value>', esc_html( $field_name ), $value );
			}
		}

		if ( ! empty( $return_fields ) ) {
			foreach ( $return_fields as $field_name ) {
				$return_fields_xml .= sprintf( $value_xml_template, esc_html( $field_name ) );
			}
		}
	...
	/**
	 * Perform the database count query to specified table with specific
	 * conditions
	 */
	public function database_count( $table_name = '', $search_fields ) {
		...
		if ( ! empty( $search_fields ) ) {
			foreach ( $search_fields as $field_name => $value ) {
				$search_fields_xml .= sprintf( '<value><struct>
						<member>
							<name>%1$s</name>
							<value><string>%2$s</string></value>
						</member>
					</struct></value>', esc_html( $field_name ), esc_html( $value ) );
			}
		}
