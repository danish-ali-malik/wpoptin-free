	...
	class CS_REST_Wrapper_Base {
		...
		function __construct(
			...
			$this->_default_call_options = [
				...
				'userAgent'   => 'createsend-php v' . CS_REST_WRAPPER_VERSION . ' PHPv' . phpversion() . ' over ' . $transport_type . ' with ' . $this->_serialiser->get_type(),
		...
		function put_request( $route, $data, $call_options = [] ) {
			return $this->_call( $call_options, CS_REST_PUT, $route, $data );
		}

		function post_request( $route, $data, $call_options = [] ) {
			return $this->_call( $call_options, CS_REST_POST, $route, $data );
		}

		function delete_request( $route, $call_options = [] ) {
			return $this->_call( $call_options, CS_REST_DELETE, $route );
		}

		function get_request( $route, $include_tracking_pref = null, $call_options = [] ) {

			if ( isset( $include_tracking_pref ) && is_bool( $include_tracking_pref ) ) {
		...
		function get_request_paged(
			$route, $page_number, $page_size, $order_field, $order_direction, $include_tracking_pref = null, $join_char = 'deprecated'
		) {
