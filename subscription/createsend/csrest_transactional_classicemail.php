...
if ( ! class_exists( 'CS_REST_Transactional_ClassicEmail' ) ) {
	class CS_REST_Transactional_ClassicEmail extends CS_REST_Wrapper_Base {
		...
		function __construct(
			$auth_details, $client_id = null, $protocol = 'https', $debug_level = CS_REST_LOG_NONE, $host = 'api.createsend.com', $log = null, $serialiser = null, $transport = null
		) {
		...
		function set_client( $client_id ) {
			$this->_client_id_param = [ "clientID" => $client_id ];
		}
		 ...
		 *      )
		 */
		function send( $message, $group = null, $consent_to_track, $add_to_list_ID = null, $options = [] ) {
			$all_params = [
				"Group"                 => $group,
				"AddRecipientsToListID" => $add_to_list_ID,
				"ConsentToTrack"        => $consent_to_track,
			];
			$data       = array_merge( $this->_client_id_param, $message, $all_params, $options );
